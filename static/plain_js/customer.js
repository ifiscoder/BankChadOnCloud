...
var userid,
	...
	first_name,
	savings_ac_no,
	savings_ac_no_masked,
	savings_ac_bal,
	checking_ac_no,
	checking_ac_no_masked,
	checking_ac_bal,
	cc_no,
	cc_no_masked,
	cc_bal;
...
function appendPrimaryData(data) {
	...
	if (
		$("#sa_card").css("display") == "none" &&
		$("#ca_card").css("display") == "none" &&
		$("#cc_card").css("display") == "none"
	) {
		console.log("none");
...
function createCheckDropdown() {
	if (
		...
		!optionExists(
			savings_ac_no,
			document.querySelector("#orderCheck_fromAccount")
		)
	) {
		var option = document.createElement("OPTION");
	...
	if (
		$("#cc_card").css("display") != "none" &&
		!optionExists(cc_no, document.querySelector("#orderCheck_fromAccount"))
	) {
		var option = document.createElement("OPTION");
			...
			if (data.message != "None" || data.message != null) {
				...
				for (var i = 0; i < data.message.length; i++) {
					checkData =
						...
						data.message[i][3] +
						", Status = " +
						(data.message[i][4] == 0 ? "Deposited" : "Active") +
						"<br />";
				}
...
$(document).ready(function () {
	...
	$('input:radio[name="satransfer"]').change(function () {
	...
	$('input:radio[name="catransfer"]').change(function () {
	...
	$("#orderCheck_btn").on("click", function () {
		...
		if (
			$("#orderCheck_amt").val() == "" ||
			$("#orderCheck_toAccount").val() == "" ||
			$("#orderCheck_fromAccount").val() == "select"
		) {
			alert("Empty Field detected!");
		} else {
			order_check(
	...
	$("#schedApp_btn").on("click", function () {
		...
		if ($(".date-picker-start").val() == "") {
			...
			schedAppoint(
				userid,
				$(".date-picker-start").val() + ", " + $(".time-picker-start").val()
			);
