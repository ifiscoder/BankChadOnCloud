...
window.addEventListener("popstate", function () {
	...
	https: history.pushState(null, null, document.URL);
});
...
key_backs.addEventListener("click", () => {
	document.querySelector("#otp_verify_input").value = document
		.querySelector("#otp_verify_input")
		.value.slice(0, -1);
});
	...
	fetch(homeURL + "sendOTP", {
		...
		.then(function (data) {
			if (data.message == "OTP Sent") {
				window.alert(
					"An OTP has been sent to your registered mobile phone no."
				);
			} else {
...
sign_up_form_1.addEventListener("submit", function (e) {
	...
	if (document.querySelector("#agree_checkbox").checked == false) {
		...
		if (
			...
			document.querySelector("#register_confirm_password").value
		) {
			container.classList.add("user-details-mode");
		} else {
...
sign_up_form_3.addEventListener("submit", function (e) {
	...
	const register_address1_data =
		document.querySelector("#register_address1").value;
	const register_address2_data =
		document.querySelector("#register_address2").value;
	...
	console.log(
		"register userid stored in local data = " + localStorage.getItem("user")
	);
	...
	fetch(homeURL + "registerCustomer", {
		...
		.then(function (response) {
			...
			if (response.redirected) {
				...
				window.alert(
					"Oops, We encountered an error, Please re-try again later :)"
				);
			}
		})
		.catch(function (error) {
...
sign_in_form.addEventListener("submit", function (e) {
	...
	if (document.querySelector("#honey_input").value != "") {
		window.alert(
			"Are you a Robot? Please fill-up the required fields manually."
		);
		...
		console.log(
			"login userid stored in local data = " + localStorage.getItem("user")
		);
		localStorage.setItem("usertype", usertype_data);
		console.log(
			"login usertype stored in local data = " +
